...
def generate_deck_for_lesson(
    *,
    ...
    catalog = catalog if catalog is not None else build_content_catalog(lesson)
    count = clamp_card_count(card_count)
    default_difficulty = "hard" if difficulty_mode == "hard" else "easy"
    ...
    raw = generate_study_coach_deck(
        ...
        page_catalog_text=format_content_catalog_for_prompt(catalog),
    )
    ...
    stamped = attach_card_metadata(
        dedupe_cards(stamp_card_ids(cards)),
        catalog,
